...
const ShortFeedCard = ({ article, index = 0, onEngagement, active = false, onAdvance, onWatched }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        console.log('[ShortFeedCard] Shared successfully');
      } else {
      ...
      console.warn('[ShortFeedCard] Share failed:', err);
    }
  };
  ...
  function scheduleAdvance() {
    ...
    timersRef.current.push(setTimeout(() => { if (onWatched) onWatched(article); if (onAdvance) onAdvance(); }, windowLen * 1000));
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`w-full h-full snap-start flex flex-col bg-black overflow-hidden relative`}>
      ...
      <div className="absolute bottom-20 right-2 flex flex-col items-center gap-6 z-20">
        ...
        {/* Share */}
        <div className="flex flex-col items-center gap-1">
          <button onClick={handleShare} className="p-3 rounded-full bg-black/40 backdrop-blur-md hover:bg-white/20 text-white transition-all active:scale-95">
            <span className="text-2xl">🔗</span>
          </button>
