...
const NewsCard = ({ article, index }) => {
  ...
  const [saved, setSaved] = useState(() => isBookmarked(article.url));
  ...
  const handleClick = async () => {
    ...
    updateProfileWithInteraction(article, engagementData);
          ...
          <div className="flex items-center justify-between pt-3 border-t border-white/5">
            ...
            <div className="flex items-center space-x-2">
              <motion.button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  const list = toggleBookmark(article);
                  setSaved(isBookmarked(article.url));
                }}
                className={`relative p-2 rounded-lg ${saved ? 'text-yellow-300' : 'text-gray-400 hover:text-yellow-400 hover:bg-yellow-400/10'} transition-colors group/bookmark`}
              >
            ...
            {/* Feedback */}
            <RecommendationFeedback article={article} />
          </div>
        </div>
      </Link>
    </motion.article>
