...
const LoginPage = () => {
  const { loginWithGoogle, loginAsGuest, isDemoMode, loginLocal, login } = useAuth(); // Destructure login here
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      try {
        await login(email, password); // Use the destructured login function
        navigate('/feed', { replace: true });
        return;
      } catch (authErr) {
        // If auth is not configured or fails, and we are in demo mode, allow local login
        // But if it's a wrong password error, we should show it.
        if (authErr.code === 'auth/wrong-password' || authErr.code === 'auth/user-not-found' || authErr.code === 'auth/invalid-credential') {
          throw authErr;
        }
        // Fallback for demo/dev environment if real auth fails for config reasons
        console.warn('Real auth failed, falling back to local/demo login:', authErr);
      }
