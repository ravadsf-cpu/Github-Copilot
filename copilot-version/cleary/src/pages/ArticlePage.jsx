            ...
            <div className="text-gray-300 leading-relaxed text-lg space-y-6">
              {article.contentHtml ? (
                <>
                  ...
                  />
                  {article.contentHtml.length < 800 && (
                    <div className="mt-8 p-6 bg-gradient-to-br from-purple-900/20 to-blue-900/20 border border-white/10 rounded-2xl text-center">
                      <h3 className="text-xl font-bold text-white mb-2">Continue Reading</h3>
                      <p className="text-gray-300 mb-4">
                        This is a preview of the full story. Access the complete article with all images and details at the source.
                      </p>
                      <a
                        href={article.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center space-x-2 px-8 py-3 bg-white text-black rounded-xl font-bold hover:bg-gray-200 transition-colors"
                      >
                ...
                <div className="space-y-6">
                  {/* Fallback for when we only have description/snippet */}
                  <p className="leading-8 text-xl text-gray-200 font-medium">
                    {article.description}
                  </p>

                  {article.content && (
                    <p className="leading-8 text-gray-300">
                      {article.content.split('[')[0]} {/* Remove [+1234 chars] suffix common in some APIs */}
                    </p>
                  )}

                  <div className="mt-8 p-8 bg-white/5 border border-white/10 rounded-2xl text-center backdrop-blur-sm">
                    <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mx-auto mb-4">
                      <ExternalLink className="w-8 h-8 text-white" />
                    </div>
                    <h3 className="text-2xl font-bold text-white mb-3">Read the Full Story</h3>
                    <p className="text-gray-400 mb-6 max-w-md mx-auto">
                      To ensure you get the most accurate and complete reporting, please visit the original publisher.
                    </p>
                    <a
                      href={article.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center space-x-2 px-8 py-4 bg-white text-black rounded-xl font-bold text-lg hover:scale-105 transition-transform"
                    >
                      <span>Open Article at {typeof article.source === 'string' ? article.source : (article.source?.name || 'Source')}</span>
                      <ExternalLink className="w-5 h-5" />
                    </a>
                  </div>
                </div>
